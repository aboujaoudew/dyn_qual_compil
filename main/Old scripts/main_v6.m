...
%%% We quotient the annotated states which share the same state and for
%%% which no singular transitions of these annotated states has been 
...
function res = main_v6(ci,MMI_sign,vect_reac,kin,nb_val,model)
clc
...
b = size(state_space);
nb_state = b(1);
display('generation of the state space done')
...
display('computation of the abstract transitions done')
...
state_space = res.ss;
trans_set = res.ts;
display('refinement with mass invariant done')
%--------------------------------------------------------------------------
%Computation of the set of transitions refined with scale separation
trans_set = trans_set_scalesep(trans_set, vect_reac, kin);
display('refinement with scale separation done')

%%--------------------------------------------------------------------------
%Computation of the set of transitions refined with the upwards crossing
%interval constraint
res = trans_set_upcrossint(state_space, trans_set, prod_ind, vect_reac, nb_val);
trans_set = res.ts;
state_space = res.ss;
display('refinement with the upwards crossing interval constraint done')
...
function res = trans_set_upcrossint(state_space, trans_set, prod_ind, vect_reac, nb_val)
%--------------------------------------------------------------------------
...
%==========================================================================
%QUOTIENTING OF THE ANNOTATED STATES FOR WHICH NO SINGULAR TRANSITIONS HAVE
%BEEN SUPPRESSED BY THE UPWARDS CROSSING INTERVAL CONSTRAINT FOR THE
%VARIABLES WHICH ARE NOT AT THEIR MAXIMUM VALUE
...
for i = 1:length(states_set_sing_uniq(:,1))
    ...
    %Determination of the variables of 'state' which are not updated by a
    %regular transition
    b = cell2mat(trans_set_reg(:,1)) - repmat(state,[length(trans_set_reg(:,1)), 1]);
    c = find(sum(abs(b')) == 0);
    cell2mat(trans_set_reg(:,1));

    list = [];
    for j = 1:length(c)
        d = cell2mat(trans_set_reg(c(j),2)) - cell2mat(trans_set_reg(c(j),1));
        e = find(d == 0);
        list = [list e];       %list of the variables which are not updated
    end
    list = unique(list);
    var_annot_not_updat = intersect(list, prod_ind);

    %error('stop')
    %Computation of the number of singular transitions which do not involve
    %variables which are not updated by a regular transition before filtering with the
    %uci constraint
    iter1 = 0;
    for j = 1:length(d_prefilt(:,1))
        e = find([cell2mat(d_prefilt(j,1)) - cell2mat(d_prefilt(j,2))] ~= 0);
        f = intersect(e, var_annot_not_updat);
        if length(f) == 0
            iter1 = iter1 + 1;
        end        
    end
    ...
    %Computation of the number of singular transitions which do not involve
    %variables which are not updated by a regular transition after filtering with the
    %uci constraint
    iter2 = 0;
...
function res = nb_sing_trans_no_uci(n)

if n ~=1 && n ~= 0
    res = n*(1 + nb_sing_trans_no_uci(n-1));
elseif n == 1
    res = 1;
end
