...
%%% Only the states which are reachable from the initial state taken into
%%% account
...
function res = stg_discr_v1_reach_uncompl(ci,MMI_sign,v,kin,nb_val)
...
b = next_states(x,v,nb_val,kin); % 1st column: next states - 2nd column: associated propensities

for i = 1: length(b(:,1)) 
    c = invar_check (MI, MMI_sign, a(i,1));
    list = [];
    if c ~= 0
        list = [list i];
    end
...
for i= 1:length(scan_states(:,1))
    for j = 1: length(b(:,1))
        if b(j,:) == scan_states(i,:)
        end
    end
